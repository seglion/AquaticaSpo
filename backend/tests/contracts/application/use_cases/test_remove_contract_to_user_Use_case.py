...
@pytest.fixture
def contract():
    return Contract(name="Contrato1", start_date="2025-01-01", end_date=None, active=True)
...
def contract_repo(contract):
    ...
    repo.remove_contract_from_user = AsyncMock(return_value=True)
    return repo
...
async def test_remove_contract_success(contract_repo, user_repo, contract, user, admin_user):
    ...
    contract_repo.remove_contract_from_user.assert_awaited_once_with(contract, user)
