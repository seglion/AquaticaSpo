...
async def test_list_contracts_for_admin():
    mock_repo = AsyncMock()
    c1 = Contract(name="Contrato 1", start_date=date.today(), end_date=None)
    c1.id = 1
    c2 = Contract(name="Contrato 2", start_date=date.today(), end_date=None)
    c2.id = 2
    contracts = [c1, c2]
...
async def test_list_contracts_for_normal_user():
    mock_repo = AsyncMock()
    c3 = Contract(name="Contrato 3", start_date=date.today(), end_date=None)
    c3.id = 3
    user_contracts = [c3]
