...
class ContractRepository:
    ...
    async def list_users_by_contract_id(self, contract_id: int) -> List[User]:
        result = await self.session.execute(select(ContractORM).where(ContractORM.id == contract_id))
        contract_orm = result.unique().scalar_one_or_none()
        if not contract_orm:
            raise ValueError("Contrato no encontrado")
        # No reutilizamos user_orm_to_domain: ese mapper también accede a
        # UserORM.contracts, que no queda cargada en 'eager' al llegar por el
        # camino Contract -> users (SQLAlchemy no encadena el joined-load en el
        # ciclo Contract<->User), lo que dispara un lazy-load fuera de contexto
        # async. No la necesitamos aquí de todas formas (UserResponseSchema no
        # incluye contratos).
        return [
            User(
                id=u.id,
