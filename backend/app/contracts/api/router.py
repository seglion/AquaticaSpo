...
from app.contracts.application.use_cases.ListUsersByContractIdUseCase import ListUsersByContractIdUseCase
from app.users.api.schemas import UserResponseSchema
...
@router.get("/{contract_id}/users", response_model=List[UserResponseSchema])
async def list_users_by_contract(
    contract_id: int,
    requester: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db_session),
):
    repo = ContractRepository(db)
    use_case = ListUsersByContractIdUseCase(repo)

    try:
        users = await use_case.execute(contract_id, requester)
    except PermissionError as e:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(e))
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
